// ===================================
// supabase.js - Elite Capital (نسخة توقعات النتائج فقط x2)
// ===================================
...
async function getDashboardStats() {
    try {
        const [
            ...
            supabaseClient.from('bets').select('*').eq('status', 'won').eq('bet_type', 'score'),
            supabaseClient.from('bets').select('*').eq('status', 'lost').eq('bet_type', 'score')
        ]);
        ...
        const pendingPredictions = predictionsRes.data || [];
        const wonPredictions = wonPredictionsRes.data || [];
        const lostPredictions = lostPredictionsRes.data || [];
        ...
        return {
            ...
            data: {
                ...
                lostPredictions: lostPredictions.length
            }
        };
    } catch (error) {
        return { success: false, error: error.message };
...
// ========== نظام توقعات النتائج (x2) فقط ==========

// وضع توقع نتيجة مباراة
async function placePrediction(predictionData) {
    try {
        // التحقق من رصيد المستخدم
        const { data: user } = await supabaseClient
            .from('users')
            .select('balance, prediction_locked, status, is_suspended')
            .eq('id', predictionData.userId)
            .single();
        
        if (user.is_suspended || user.status === 'suspended' || user.status === 'banned') {
            throw new Error('التوقعات مقفلة لهذا الحساب');
        }
        
        if (user.prediction_locked) {
            throw new Error('التوقعات مقفلة لهذا الحساب');
        }
        
        if (user.balance < predictionData.amount) {
            throw new Error('الرصيد غير كافي');
        }
        
        if (predictionData.amount < 5) {
            throw new Error('الحد الأدنى للتوقع 5$');
        }
        ...
        if (!match) {
            throw new Error('هذه المباراة غير متاحة للتوقع');
        }
        
        // التحقق من أن المباراة لم تبدأ
        if (new Date(match.match_date) < new Date()) {
            throw new Error('لا يمكن التوقع على مباراة بدأت بالفعل');
        }
        
        // التحقق من صحة النتيجة المدخلة
        if (predictionData.predictedHome < 0 || predictionData.predictedHome > 20 || 
            predictionData.predictedAway < 0 || predictionData.predictedAway > 20) {
            throw new Error('النتيجة يجب أن تكون بين 0 و 20');
        }
        
        // حساب الربح المحتمل (مضاعف 2x)
        const odds = 2.0;
        const potentialWin = predictionData.amount * odds;
        ...
        const { data: prediction, error } = await supabaseClient
            ...
            .insert([{
                ...
                bet_type: 'score',
                bet_details: {
                    predicted_home: predictionData.predictedHome,
                    predicted_away: predictionData.predictedAway
                },
        ...
        await supabaseClient
            .from('users')
            .update({ 
                balance: user.balance - predictionData.amount,
                prediction_balance: (user.prediction_balance || 0) + predictionData.amount
            })
            .eq('id', predictionData.userId);
        ...
        await supabaseClient
            ...
            .update({
                ...
                total_bet_amount: (match.total_bet_amount || 0) + predictionData.amount
            })
            .eq('id', predictionData.matchId);
        ...
        await supabaseClient
            ...
            .insert([{
                ...
                type: 'توقع نتيجة',
                amount: -predictionData.amount,
                description: `توقع نتيجة ${match.home_team} vs ${match.away_team}: ${predictionData.predictedHome}-${predictionData.predictedAway}`,
                status: 'completed',
                bet_id: prediction.id,
                created_at: new Date().toISOString()
            }]);
        ...
        await addActivity({
            ...
            type: 'score_bet',
            title: '🎯 توقع نتيجة',
            description: `توقعت نتيجة ${match.home_team} vs ${match.away_team}: ${predictionData.predictedHome}-${predictionData.predictedAway}`,
            amount: predictionData.amount,
            status: 'pending'
        });
...
async function settlePredictions(matchId, actualHome, actualAway) {
    try {
        // جلب المباراة
        const { data: match } = await supabaseClient
            .from('matches')
        ...
        if (!predictions || predictions.length === 0) return { success: true };
        
        for (const pred of predictions) {
            const details = pred.bet_details;
            
            // التحقق من تطابق النتيجة
            if (details.predicted_home === actualHome && details.predicted_away === actualAway) {
                // توقع صحيح - ربح x2
                const winAmount = pred.amount * 2;
                ...
                const { data: user } = await supabaseClient
                    .from('users')
                    .select('balance, prediction_balance')
                    .eq('id', pred.user_id)
                    .single();
                ...
                await supabaseClient
                    ...
                    .update({ 
                        ...
                        prediction_balance: (user.prediction_balance || 0) - pred.amount
                    })
                    .eq('id', pred.user_id);
                ...
                await updatePredictionStats(pred.user_id, pred.amount, winAmount, true);
                ...
                await updatePredictionStats(pred.user_id, pred.amount, 0, false);
        ...
        console.error('خطأ في تسوية التوقعات:', error);
        return { success: false, error: error.message };
    }
}

// تحديث إحصائيات التوقعات للمستخدم
async function updatePredictionStats(userId, amount, wonAmount, isWinner) {
    try {
        ...
        console.error('خطأ في تحديث إحصائيات التوقعات:', error);
    }
}

// جلب توقعات المستخدم
async function getUserPredictions(userId, status = null, limit = 50) {
    try {
        let query = supabaseClient
            .from('bets')
            .select(`
...
// جلب إحصائيات التوقعات للمستخدم
async function getUserPredictionStats(userId) {
    try {
        const { data, error } = await supabaseClient
            .from('betting_stats')
            .select('*')
...
// جلب جميع التوقعات (للإدارة)
async function getAllPredictions(status = null, limit = 100) {
    try {
        let query = supabaseClient
            .from('bets')
            .select(`
...
async function getPredictionStats() {
    try {
        ...
        const totalWagered = predictions.reduce((sum, p) => sum + (p.amount || 0), 0);
        const totalPayout = predictions
            .filter(p => p.status === 'won')
            .reduce((sum, p) => sum + (p.potential_win || 0), 0);
...
console.log('✅ تم تحميل جميع دوال Supabase مع نظام توقعات النتائج x2');
