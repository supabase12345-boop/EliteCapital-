...
function getMatchTimeRemaining(matchDateString) {
    ...
    // تحويل الوقت الحالي إلى UTC
    const nowUTC = new Date(now.getTime() - (now.getTimezoneOffset() * 60000));
    
    // الفرق بالدقائق
    const diffMinutes = (matchDate - nowUTC) / (1000 * 60);
...
async function getAllMatches() {
    try {
        ...
        // تحويل الوقت عند العرض
        const formattedData = data?.map(match => ({
            ...match,
            match_date: match.match_date,
            local_time: formatLocalTime(match.match_date),
            time_remaining: getMatchTimeRemaining(match.match_date)
        }));
...
// إضافة مباراة جديدة (مع تصحيح الوقت)
async function addMatch(matchData) {
    try {
        // تحويل الوقت المدخل إلى UTC قبل الحفظ
        const utcDate = convertToUTC(matchData.match_date);
        ...
        for (const bet of bets || []) {
            ...
            if (betStatus === 'won' && paidAmount > 0) {
                ...
                await supabaseClient
                    .from('users')
                    .update({
                        balance: bet.users.balance + paidAmount,
                        updated_at: new Date().toISOString()
                    })
                    .eq('id', bet.user_id);
                ...
                await addActivity({
                    ...
                    type: 'profit',
                    title: '🏆 فوز في الرهان',
                    description: `ربح ${paidAmount}$ من الرهان`,
                    amount: paidAmount
                });
            }
        }
...
async function placeBet(betData) {
    try {
        ...
        // التحقق من وقت المباراة (باستخدام الوقت الحالي)
        const now = new Date();
        ...
        await supabaseClient
            ...
            .insert([{
                ...
                type: 'رهان',
                amount: -betData.amount,
                description: `رهان على مباراة ${match.team1} vs ${match.team2}`,
                status: 'pending',
                bet_id: bet.id,
                created_at: new Date().toISOString()
            }]);
...
// الحصول على جميع الرهانات (للمشرف)
        ...
        const sampleMatches = [
            {
                ...
                match_date: new Date(now.getTime() + 2 * 60 * 60 * 1000) // بعد ساعتين
            },
            {
                team1: 'برشلونة',
                team2: 'ريال مدريد',
                league: 'الدوري الإسباني',
                match_date: new Date(now.getTime() + 5 * 60 * 60 * 1000) // بعد 5 ساعات
            },
            {
                team1: 'ليفربول',
                team2: 'مانشستر سيتي',
                league: 'الدوري الإنجليزي',
                match_date: new Date(now.getTime() + 8 * 60 * 60 * 1000) // بعد 8 ساعات
            },
            {
                team1: 'بايرن ميونخ',
                team2: 'بوروسيا دورتموند',
                league: 'الدوري الألماني',
                match_date: new Date(now.getTime() + 12 * 60 * 60 * 1000) // بعد 12 ساعة
            },
            {
                team1: 'يوفنتوس',
                team2: 'ميلان',
                league: 'الدوري الإيطالي',
                match_date: new Date(now.getTime() + 24 * 60 * 60 * 1000) // بعد يوم
            },
            {
                team1: 'باريس سان جيرمان',
                team2: 'مارسيليا',
                league: 'الدوري الفرنسي',
                match_date: new Date(now.getTime() + 30 * 60 * 60 * 1000) // بعد 30 ساعة
            },
            {
                team1: 'الأهلي',
                team2: 'الهلال',
                league: 'دوري أبطال آسيا',
                match_date: new Date(now.getTime() + 36 * 60 * 60 * 1000) // بعد 36 ساعة
            },
            {
                team1: 'النصر',
                team2: 'الاتحاد',
                league: 'الدوري السعودي',
                match_date: new Date(now.getTime() + 48 * 60 * 60 * 1000) // بعد يومين
            }
        ];
...
console.log('✅ تم تحميل جميع دوال Supabase مع نظام المراهنات وحل مشكلة الوقت');
