// ===================================
// supabase.js - Elite Capital (نسخة محدثة بالكامل مع نظام المراهنات المبسط)
// ===================================
...
// ========== نظام المراهنات المبسط ==========

// إضافة مباراة جديدة
async function addMatch(matchData) {
    try {
        console.log('محاولة إضافة مباراة:', matchData);
        ...
        console.log('تم إضافة المباراة بنجاح:', data);
        return { success: true, data };
    } catch (error) {
        console.error('خطأ في addMatch:', error);
        return { success: false, error: error.message };
    }
}
...
async function placeBet(betData) {
    try {
        ...
        // تحديد نسبة الربح حسب نوع الرهان
        let odds = 1.6; // افتراضي للفوز/تعادل
        if (betData.betType === 'exact') {
            odds = 2.5; // للنتيجة الدقيقة
        }
...
// إنهاء مباراة وتحديد النتائج (مبسطة)
    ...
    try {
        ...
        for (const bet of bets) {
            ...
            if (bet.bet_type === 'simple') {
                ...
                // رهان نتيجة دقيقة
                if (bet.bet_value === `${team1Score}-${team2Score}`) isWinner = true;
            }
            ...
            if (isWinner) {
                ...
                await supabaseClient
                    ...
                    .update({
                        ...
                        total_earned: (bet.users.total_earned || 0) + winAmount
                    })
                    .eq('id', bet.user_id);
...
console.log('✅ تم تحميل جميع دوال Supabase مع نظام المراهنات المبسط');
