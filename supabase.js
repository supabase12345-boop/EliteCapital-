// ===================================
// supabase.js - Elite Capital (نسخة محدثة مع الهدايا والدردشة بالصور)
// ===================================
...
// ========== نظام الدردشة مع دعم الصور ==========
async function uploadChatImage(file, chatId, userId) {
    try {
        console.log('📸 بدء رفع صورة:', file.name);
        ...
        const fileName = `chat_${chatId}_${userId}_${Date.now()}.${fileExt}`;
...
async function sendChatMessageWithImage(chatId, userId, message, imageFile = null) {
    try {
        let imageUrl = null;
        
        if (imageFile) {
            const uploadResult = await uploadChatImage(imageFile, chatId, userId);
        ...
        console.log('إرسال رسالة:', { chatId, userId, message, imageUrl });
...
// ========== نظام الهدايا من الإدارة ==========
async function sendGift(giftData) {
    try {
        console.log('🎁 بدء إرسال هدية:', giftData);
        
        if (!giftData.userId) throw new Error('معرف المستخدم مطلوب');
        if (!giftData.amount || giftData.amount <= 0) throw new Error('مبلغ الهدية يجب أن يكون أكبر من 0');
        if (!giftData.adminId) throw new Error('معرف المسؤول مطلوب');
        
        const { data: user, error: userError } = await supabaseClient
            .from('users')
            .select('balance, name, email')
            .eq('id', giftData.userId)
            .single();
        ...
        const { data: admin, error: adminError } = await supabaseClient
            .from('users')
            .select('name')
            .eq('id', giftData.adminId)
            .single();
        
        if (adminError || !admin) throw new Error('المسؤول غير موجود');
        
        const newBalance = (user.balance || 0) + giftData.amount;
        ...
        const { data: transaction, error: transError } = await supabaseClient
            ...
            .insert([{
                ...
                description: giftData.reason || '🎁 هدية من الإدارة',
                status: 'completed',
                created_at: new Date().toISOString()
            }])
            .select()
        ...
        await addActivity({
            ...
            description: giftData.reason || `تم إضافة ${giftData.amount}$ كهدية من الإدارة`,
            amount: giftData.amount,
            status: 'completed'
        });
        ...
        await supabaseClient
            ...
            .insert([{
                user_id: giftData.userId,
                title: '🎁 هدية من الإدارة',
                message: giftData.reason || `تم إضافة ${giftData.amount}$ إلى رصيدك كهدية من الإدارة`,
                type: 'gift',
                is_read: false,
                created_at: new Date().toISOString()
            }]);
        
        console.log('✅ تم إرسال الهدية بنجاح');
...
console.log('✅ تم تحميل جميع دوال Supabase مع إضافة الهدايا والدردشة بالصور');